...
`ripples`) plus a `gustRate` companion and a `RAIN_PRESETS` export. The hot body
gains exactly four guarded adds: a once-per-frame `_elapsed += dt`, an
`if (gust !== 0)` frame-top windPulse folded into the state-1 `vx`, the floor line
folded into a single `fy` local, and two `if` gates (`splashDroplets !== 0`, ripple
ring non-null) around cold blocks. The OFF path -- `gust:0`, `splashDroplets:0`,
`ripples:false`, `floorY:null` -- is **bit-identical** to v1.2.0: verified against a
COMMITTED v1.2.0 byte-golden (`test/torture/fixtures/v1.2.0-off-golden.json`,
captured from the actual 1.2.0 source at commit 9aaa783 -- raw-byte equality of
x/y/vx/vy/state/life after a 400-frame seeded run), and it carries no ripple buffers
at all. The T5 full-scan draw-set oracle separately proves the ON-feature render
draws each live particle exactly once. Both feature states, ON and OFF, are
zero-allocation (proven by T6's two windows).
Proven by the torture harness (`node --expose-gc test/torture.mjs` -> `ok`,
`alloc` ~0.11 B/op, `gc major=0`).

### Changed

- **Floor line folded into `fy` (a single frame-top local).** `updateAndDraw`
  derived the splash floor from the frame height `h` in two places (the state-1
  floor-hit test and the state-2 clamp). Both now read `const fy = floorY !== null
  ? floorY : h`. With the default `floorY:null` this is exactly `h` -- **no visible
  change by default**. The horizontal/top positional-cull margins (`-200`, `w+200`,
  `y >= -200`) are unchanged: only the FLOOR line moved.
- **`_elapsed` accumulator advances every frame, unconditionally.** The gust phase
  clock is not gated on the feature; only the READ that consumes it
  (`if (gust !== 0)`) is. This is the determinism contract -- the clock is a pure
  function of the frame sequence, independent of whether gust is on.

### Added

- **`gust` config (default `0`) + `gustRate` (default `Math.fround(2*PI/3)`, ~3s
  period).** A live wind gust: `vx += sin(_elapsed * gustRate) * gust * dt` per
  state-1 frame, computed once at frame top (0 when `gust:0`). Both are validated by
  the finite-config door.
- **`floorY` config (default `null`).** The splash floor Y; `null` is the documented
  "use the frame height `h`" sentinel. Null-or-finite (a non-finite non-null value
  throws naming the key).
- **`splashDroplets` config (default `0`), an integer in `0..3`.** Emits N tiny
  state-2 droplets per impact into free pool slots via the shared spawn ring,
  respecting `max` (no free slot -> no droplet). The ring read/write is strictly
  inside the `splashDroplets !== 0` gate, so the OFF path never perturbs slot
  assignment. A droplet is binned exactly once (in emission if it lands behind the
  scan cursor, else by the main pass), so the full-scan draw-set oracle still holds.
- **`ripples` config (default `false`), a STRICT boolean.** Expanding ground ripple
  rings, one per impact, written into a pre-allocated 64-slot Float32 ring
  (allocated ONLY when `ripples:true`; the three fields stay `null` otherwise). All
  live rings are aged and stroked as ellipses in ONE `beginPath()/stroke()` after
  the splash loop, fully independent of the pool. The ring never grows and is nulled
  by `destroy()`.
- **`RAIN_PRESETS` export.** Frozen partial-config presets `drizzle`, `steady`,
  `downpour`, `storm` (storm = high wind + slanted `angle` + live `gust`). Every
  value is finite and passes the door when spread into config.

### Fixed

- None. R3 is purely additive; no defect from v1.2.0 is changed.

### Testing

- **T8** gains the cross-version OFF-path golden -- the current R3 engine on its OFF
  path reproduces the committed v1.2.0 byte-golden (`fixtures/v1.2.0-off-golden.json`,
  regenerated by `fixtures/generate-golden.mjs` from commit 9aaa783) -- and the gust
  oscillator: a single frozen drop's `vx` rises then FALLS across a period (the
  windPulse increment changes sign each half-period), `gustRate * 2` halves the
  period, and `gustRate:0` freezes it at exactly `0`.
- **T9** Control 7 proves the golden is sensitive to the exact mutation the gates
  prevent: a clean OFF run matches the golden, but injecting one spurious emit (a
  forced state-2 slot + a spawn-cursor advance) makes the snapshot FAIL it.
- **T6** adds a second window with ALL features on and pins the ripple ring's
  `length === 64` and `byteLength` across a 500k-op steady state.
- **T5** adds `gust storm`, `droplets`, `ripples`, and `floorY` fuzz scenarios; the
  oracle reads `floorY` for its floor line. **T7** soaks an all-features-on engine
  every other cycle (ripple ring build/teardown must be reclaimed). **T1** extends
  the door matrix to `gust`, `gustRate`, `floorY`, `splashDroplets`, `ripples`.
- 63 unit tests (`node --test`) green.
